    ...
    def download(self, url, filename, unzip=False):
        ...
        if unzip:
            ...
            with zipfile.ZipFile(filename) as zip_ref:
                zip_ref.extractall(os.path.dirname(filename))
            # else:
            #    with tarfile.open(filename, "r:gz", encoding='utf8') as tar_ref:
            #        tar_ref.extractall(filename+"[UZ]")
        #    BackgroundJob.get_instance().set_status(task_id, "status", "done")
            # except:
            #    BackgroundJob.get_instance().set_status(task_id, "status", "error")

    @app.task(bind=True)
    def download_model(self, url, filename, unzip=False):
        print("Downloading...")
        os.mkdir(filename)
        task_id = self.request.id
        print(task_id)
        ...
        if unzip:
            ...
            # try:
            # if filename.endswith(".zip"):
            with zipfile.ZipFile(filename+"-[DOWNLOAD]") as zip_ref:
